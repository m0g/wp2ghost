...
var WpShortcode = require('./wp-shortcode');


var performDownload = function(src, dest) {
...
var downloadImage = function(data) {
  var date = data.src.match(/(\d{4})\/(\d{2})\/(\S+\.jpg)/i);
  var year = './images/' + date[1];
  var month = year + '/' + date[2];
  var filename = month + '/' + date[3];
  ...
  return performDownload(data.src, data.dest);
};
...
var downloadImages = function(images) {
  ...
  return images.map(function(image) {
    var date = image.match(/(\d{4})\/(\d{2})\/(\S+\.jpg)/i);
    var year = './images/' + date[1];
    var month = year + '/' + date[2];
    var filename = month + '/' + date[3];

    return { src: image, dest: filename };
  });
};

var getFileName = function(src) {
  var date = src.match(/(\d{4})\/(\d{2})\/(\S+\.jpg)/i);
  var year = './images/' + date[1];
  var month = year + '/' + date[2];
  var filename = month + '/' + date[3];

  return { year: year, month: month, filename: filename };
};
...
var treat = function(html) {
  ...
  // TODO: download images to a corresponding folder
  var imgSrc = html.match(/(https:\/\/adambemma\.files\.wordpress\.com\/\d{4}\/\d{2}\/\S+\.jpg)/g);

  if (imgSrc) {
    imgSrc.forEach(function(imageSrc) {
      var dest = '/content' + getFileName(imageSrc).filename.substring(1);
      html = html.replace(imageSrc, dest);
    })
  }

  //if (html.match(/Kedit/i))
  //  console.log('html', html);

  var wpShortcode = new WpShortcode(html);
  wpShortcode.replace('soundcloud');
  html = wpShortcode.html;

  html = toMarkdown(html, { filter: [ 'img', 'p', 'strong', 'em', 'br' ] });

  //if (html.match(wpShortcode.regExp('soundcloud')))
  //  console.log('before shortcode', html.match(wpShortcode.regExp('soundcloud'))[0]);
  //Shortcode.replace('soundcloud', html, function(arg, ument) {
  //  console.log('hi replace', arg, ument);
  //});
  ...
  // TODO: Rename image to the following style: /content/images/2016/01/20151222_154708-01.jpeg

  // TODO: Transform image tag into markdown
...
exports.fromStream = function(stream, withImages) {
  ...
  return new Promise(function(resolve, reject) {
    ...
    xml.on('endElement: item', function(item) {
      ...
      var html = item['content:encoded'].$children.join('');

      var imgSrc = html
        .match(/(https:\/\/adambemma\.files\.wordpress\.com\/\d{4}\/\d{2}\/\S+\.jpg)/g);

      if (imgSrc) downloadQueue.push(downloadImages(imgSrc));
    ...
    xml.on('end', function() {
      ...
      downloadQueue = _.flatten(downloadQueue);
      ...
      if (!withImages)
        ...
        Promise.each(downloadQueue, downloadImage)
          .then(function() {
            resolve(ghost);
          });
    });
...
exports.fromFile = function(file, withImages) {
  var fs = require('fs');

  return exports.fromStream(fs.createReadStream(file), withImages);
};
